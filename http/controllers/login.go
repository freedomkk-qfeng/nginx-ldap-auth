...
func (this *LoginController) Get() {
	...
	loginFailed := this.GetString("loginFailed")
	if loginFailed != "" {
...
func (this *LoginController) Post() {
	...
	if len(g.Config().Control.AllowUser) > 0 {
		if !utils.In_slice(username, g.Config().Control.AllowUser) {
			...
			this.Ctx.Redirect(302, fmt.Sprintf("/login?loginFailed=2&target=%s", target))
			return
		}
	}
	...
	if err == nil {
		...
		this.Ctx.Redirect(302, fmt.Sprintf("/login?loginFailed=1&target=%s", target))
	}
}
